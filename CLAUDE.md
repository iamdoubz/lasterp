...
Go **1.26.5** pinned — track latest stable patch within 2 weeks of release (server, **no CGO** — modernc.org/sqlite, wazero), PostgreSQL 18+/SQLite behind the storage adapter, NATS JetStream (embedded lib in solo mode), React 19 + TypeScript + Vite + TanStack (web), Extism/wazero (plugins), MCP (AI surface).
