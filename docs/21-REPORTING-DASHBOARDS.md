...
- **WP-1.8 Role packs v1 — done:** (CEO/CFO/AR, plus a gated AP pack) + KPI cards with mandatory comparisons. AC: fresh tenant → role dashboard live with seed data; 5-second-headline heuristic reviewed per pack.
  - **Packs are embedded YAML** ([`modules/reporting/packs`](../modules/reporting/packs)) declaring a headline tile, supporting tiles, and the roles they suit. A pack is *available* when the viewer may evaluate its headline metric and *suggested* when it matches one of their roles — role names order the list, permissions decide what renders (INV-T1). Tiles a viewer may not see are named as omitted, never rendered as zeroes. The AP pack ships gated on the `payables` capability until WP-4.1.
  - **Comparisons required a period-aware metrics layer.** Ledger metrics evaluated the all-time projection, so every "vs prior period" delta would have been exactly zero. Metrics now declare a **grain** — *flow* (revenue, expenses: movement within a period) or *stock* (cash, assets, AR: balance as at a period end) — and fold on the entry's fiscal **period**, not wall-clock time. `ledger.BalancesForPeriods` is verified against the materialized projection (INV-E5).
  - **Cards** carry value, comparison basis, delta (absolute + basis points, integer — no float on a money path), and the metric's `good_direction`; the sign and an arrow carry the meaning so colour never has to. Sparklines and charts wait for WP-4.13 and its charting ADR.
  - **`lasterp demo`** seeds a small book through the ordinary posting pipeline (INV-X5), which is what makes "fresh tenant shows a live dashboard" true outside a test. Decisions: [docs/notes/WP-1.8-decisions.md](notes/WP-1.8-decisions.md).
