...
## Build plan
- **WP-0.8 Integrity foundation (Phase 0, blocks all module work):** invariant registry (catalog as code), gauntlet CI stage skeleton, adversarial writer suite v1, DB role separation, append-only enforcement. AC: all INV-E/T invariants have tagged failing-by-default tests that pass only with enforcement in place.
