...
**Status 2026-07-30: Phase 0 is COMPLETE** (see [docs/notes/phase-0-review.md](notes/phase-0-review.md)) **and Phase 1 is through WP-1.5: WP-1.0a, 1.1, 1.2, 1.3, 1.4 and 1.4b are merged** (PRs #11–18) — see [docs/notes/phase-1-midpoint.md](notes/phase-1-midpoint.md) for the mid-phase review. WP-1.5 (web client v1) is in review; the product is now usable end-to-end from a browser. Next up: WP-1.6 (reports + metrics + AR receipts). The critical path was/is:
...
- **WP-1.6 Reports v1 + metrics layer** — **split into two PRs** (see [decisions](notes/WP-1.6-decisions.md)): **PR-A (WP-1.6a) AR receipts + settlement** — the invariant-bearing half, landing the new **INV-F8** (settlement never exceeds the document it settles); **PR-B reports + metrics + export** — read-only, reconciled against the event-fold oracle. Settlement state is *derived* from receipts, never written onto the posted invoice (INV-F2). Amended per [21-REPORTING-DASHBOARDS.md](21-REPORTING-DASHBOARDS.md): trial balance, P&L, balance sheet, AR aging from projections; CSV/XLSX export; metrics layer v1 + drill-down. **Includes minimal AR receipt recording** (payment → GL via declared template + invoice paid/partial status): no other WP owns it, aging is degenerate without payment-reduced balances, and M1 ("keep books") implies booking customer payments — full payment matching/dunning stays in Phase 4 banking. AC: report/metric totals reconcile with event-fold oracle; permission-leak suite green; recorded receipt reduces AR aging and flips invoice status.
