...
- **WP-1.6 Reports v1 + metrics layer** (amended per [21-REPORTING-DASHBOARDS.md](21-REPORTING-DASHBOARDS.md)): trial balance, P&L, balance sheet, AR aging from projections; CSV/XLSX export; metrics layer v1 + drill-down. **Includes minimal AR receipt recording** (payment → GL via declared template + invoice paid/partial status): no other WP owns it, aging is degenerate without payment-reduced balances, and M1 ("keep books") implies booking customer payments — full payment matching/dunning stays in Phase 4 banking. AC: report/metric totals reconcile with event-fold oracle; permission-leak suite green; recorded receipt reduces AR aging and flips invoice status.
