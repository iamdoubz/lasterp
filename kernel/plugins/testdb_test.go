...
// testDialects returns one migrated *storage.DB per dialect the vault must
// pass on (CLAUDE.md: "adapter conformance suite must pass on Postgres AND
// SQLite for storage-touching code").
...
// testPostgresDB migrates as the container superuser, then hands back a
// connection as an ordinary NOSUPERUSER NOBYPASSRLS role — row security is
// never applied to superusers, so testing tenant isolation through a superuser
// connection would make every RLS assertion a false positive (the mistake
// WP-0.3's tests originally made; see WP-0.4-decisions.md).
func testPostgresDB(t *testing.T) *storage.DB {
	t.Helper()
	ctx := context.Background()

	container, err := tcpostgres.Run(ctx, "postgres:18",
		tcpostgres.WithDatabase("lasterp_plugins"),
