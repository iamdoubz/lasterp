...
func testSQLiteDB(t *testing.T) *storage.DB {
	t.Helper()
	dsn := filepath.Join(t.TempDir(), "money.db") + "?_pragma=busy_timeout(30000)"
...
// testPostgresDB migrates as the testcontainers superuser, then returns a
// connection as an ordinary NOSUPERUSER NOBYPASSRLS role so RLS is actually
// exercised (the WP-0.4 lesson: superuser bypasses RLS and hides isolation
// bugs).
func testPostgresDB(t *testing.T) *storage.DB {
	...
	container, err := tcpostgres.Run(ctx, "postgres:18",
		tcpostgres.WithDatabase("lasterp_money"),
